...
async def themes_handler(message: types.Message, state: FSMContext):
    ...
    if message.text == 'Тема 1':
        ...
        generator.start()
        await bot.send_photo(user_id, open('photo/temp1.png', 'rb'))
        await message.answer(ss.questions1)
        await StateGroup.wait_for_answer.set()
    elif message.text == 'Тема 2':
        await message.answer(text='Начнем!', reply_markup=bs.gameMenu)
        generator.start()
        await bot.send_photo(user_id, open('photo/temp2.png', 'rb'))
        await message.answer(ss.questions1)
        await StateGroup.wait_for_answer.set()
    elif message.text == 'Тема 3':
        await message.answer(text='Начнем!', reply_markup=bs.gameMenu)
        generator.start()
        await bot.send_photo(user_id, open('photo/temp3.png', 'rb'))
        await message.answer(ss.questions1)
        await StateGroup.wait_for_answer.set()
    elif message.text == 'Тема 4':
        await message.answer(text='Начнем!', reply_markup=bs.gameMenu)
        generator.start()
        await bot.send_photo(user_id, open('photo/temp4.png', 'rb'))
        await message.answer(ss.questions1)
        await StateGroup.wait_for_answer.set()
    elif message.text == 'Тема 5':
        await message.answer(text='Начнем!', reply_markup=bs.gameMenu)
        generator.start()
        await bot.send_photo(user_id, open('photo/temp5.png', 'rb'))
        await message.answer(ss.questions1)
        await StateGroup.wait_for_answer.set()
